    ...
    <aside className="w-64 bg-teal-600/90 backdrop-blur-md p-4 min-h-[calc(100vh-140px)] border-r border-teal-500/30">
      ...
      <div className="relative mb-4">
        <input
          ...
          className="w-full px-4 py-2 pr-10 rounded-lg bg-white/20 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/40 transition-all"
        />
        <button
          onClick={handleSearchClick}
          className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-white/80 hover:text-white hover:bg-white/10 rounded transition-colors"
          title="Tìm kiếm"
        >
      ...
      <div className="mb-4 p-2 bg-white/10 rounded-lg border border-white/20">
        <p className="text-white/80 text-xs">
          💡 Nhập tên thành phố (VD: Tokyo, Paris) và nhấn Enter
        </p>
      </div>
      ...
      <div className="space-y-2 max-h-[calc(100vh-400px)] overflow-y-auto custom-scrollbar">
        {filteredCities.length > 0 ? (
          <>
            ...
            {filteredCities.filter(city => cities.includes(city)).map(city => (
              <button
                key={city}
                onClick={() => {
